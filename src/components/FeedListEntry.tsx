...
const FeedListEntry: React.FC<FeedListEntryProps> = ({
  ...
  return (
    ...
    >
      ...
      {isMobile && (
        <div className={`absolute inset-y-0 right-0 flex items-center gap-3 pr-4 ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'}`}
          style={{ width: 156 }}
        >
          <button
            onClick={(e) => {
              e.stopPropagation();
              onToggleStar(currentEntry.id!);
              resetReveal();
            }}
      ...
      <div className="flex items-center px-4 py-2 gap-4">
        ...
        <div className="flex items-center gap-1">
          ...
          {(currentEntry.interestScore ?? 0) > 0 && (
            <div
              className={`px-1.5 py-0.5 rounded text-xs font-medium
                ${isDarkMode
                  ? 'bg-purple-500/20 text-purple-200'
                  : 'bg-purple-100 text-purple-800'}`}
              title={`Interest score: ${currentEntry.interestScore}`}
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor">
                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
              </svg>
